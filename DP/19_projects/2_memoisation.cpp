...
long long finder(int ind, vector<Project> &p_list, vector<long long> &dp)
{
    if (ind >= p_list.size())
        return 0;
    ...
    select += finder(next_idx, p_list, dp);

    long long not_select = 0 + finder(ind + 1, p_list, dp);

    return dp[ind] = max(select, not_select);
}
